...
async function main() {
    ...
    giaVaoLenh = 0
    Best = 0
    // a = await future.setMarginMode('isolated', '1000PEPEUSDT', undefined)
    // await future.fetchPositions('1000PEPEUSDT', undefined)
    // console.log(a)
    while(true){
        if(inPosition) {
            // await delay(10*200)
        ...
        // console.log(a)
        const price = await future.fetchOHLCV('PEPEUSDT', '1s', undefined, 1)       
        const _1000PEPEPrice = price.map(price => {
            return {
                timeStamp: moment(price[0]).format(),
                open: price[1], low: price[3], high: price[2], close: price[4], vol: price[5]
            }
        })
        curPr = _1000PEPEPrice[0].open
        
        // if (inPosition) {
            console.log('Gia hien tai: ', parseFloat(_1000PEPEPrice[0].open).toPrecision(12))
        // }
        if (first) {
            first = false
            lastP = _1000PEPEPrice[0].open
        }
        else{
            
            change = Math.max(lastP, _1000PEPEPrice[0].open) - Math.min(_1000PEPEPrice[0].open, lastP)
            if (change != 0){
                if(inPosition){
                    ...
                    if(count>30){
                        if(count%30==0){
                            console.log('1000PEPE - eve: ', parseFloat(eve*100).toPrecision(4), '% - current price: ', _1000PEPEPrice[0].open)
                        }
                        if(change/lastP>eve*8){
                            console.log('Warning at: ', _1000PEPEPrice[0].timeStamp)
                            console.log('eve: ', parseFloat(eve*100).toPrecision(4), ' lastP: ',
                             lastP, ' - curP: ', _1000PEPEPrice[0].open, 'change: ', Math.ceil(change*100000), 'eveNo: ', Math.ceil(eveNo))
                            if(curPr < lastP){
                                console.log('Mua tai: ', curPr)
                                side =1
                            }
